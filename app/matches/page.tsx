    ...
    <div className="bg-white rounded-2xl shadow-sm border border-amber-100 p-4">
      <div className="flex items-center gap-3 mb-3">
        ...
        <div className="flex-1">
          <p className="font-semibold text-gray-800">{otherUser?.displayName ?? 'Loading...'}</p>
      ...
      {isIncoming && (
        <div className="flex gap-2">
          ...
          <button
            onClick={onAccept}
            disabled={isExpired}
            className="flex-1 bg-amber-500 hover:bg-amber-600 disabled:opacity-50 disabled:cursor-not-allowed text-white py-2 rounded-xl text-sm font-semibold transition-colors"
          >
            Accept Walk! 🚶
          </button>
        </div>
      ...
      {match.status === 'accepted' && (
        <Link
          ...
          className="block w-full text-center bg-amber-50 hover:bg-amber-100 text-amber-700 font-semibold py-2 rounded-xl text-sm transition-colors"
        >
          💬 Open Chat
        </Link>
      )}
  ...
  return (
    <div className="min-h-screen bg-amber-50 pb-24">
      <div className="bg-white border-b border-amber-100 px-4 py-4">
        <h1 className="text-xl font-bold text-gray-800">Walk Matches</h1>
        <p className="text-sm text-gray-500">Manage your walking plans</p>
      </div>
      ...
      <div className="max-w-sm mx-auto px-4 py-4 space-y-6">
        {loading ? (
          <div className="space-y-3">
            {[1, 2].map((i) => (
              <div key={i} className="bg-white rounded-2xl h-32 animate-pulse border border-amber-100" />
            ))}
          </div>
        ) : (
          <>
            ...
            {matches.length === 0 && (
              <div className="text-center py-12">
                ...
                <p className="text-gray-400 text-sm mt-1">
                  Head to the map to find walking companions!
                </p>
                <Link
                  href="/map"
